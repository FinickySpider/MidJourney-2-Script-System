...
# Global configuration values (initially from config.ini)
...
# ------------------------------
# Prompt Expansion Function (Updated to allow all symbols allowed in file names)
# ------------------------------
RECURSION_DEPTH = config.getint("Settings", "RecursionDepth", fallback=5)
def expand_prompt(template, depth=RECURSION_DEPTH):
    if depth <= 0:
        return template
    # Updated regex: match anything inside [ ... ] except the closing bracket
    pattern = r"\[([^\]]+)\]"
    def replace(match):
        # Normalize key to uppercase for dictionary lookup
        key = match.group(1).upper()
        if key in wildcards:
            replacement = random.choice(wildcards[key])
            # Recursively expand if the replacement contains further wildcards
            return expand_prompt(replacement, depth-1)
        else:
            return match.group(0)  # Return unchanged if key not found
...
async def handler(websocket, path):
    ...
    try:
        async for message in websocket:
            logging.info(f"Minerva: Received message from client (ignored): {message}")
...
async def prompt_generator():
    global current_prompt_template, current_message_send_delay
    while not stop_event.is_set():
        prompt = expand_prompt(current_prompt_template)
        logging.info(f"Minerva: Generated prompt: {prompt}")
        if connected_clients:
            try:
                await asyncio.gather(*(client.send(prompt) for client in connected_clients))
                logging.info("Minerva: Prompt sent to clients.")
            except Exception as e:
                logging.error("Minerva: Error sending prompt: " + str(e))
        else:
            logging.warning("Minerva: No connected clients to send prompt.")
        try:
...
async def run_server():
    ...
    # Run prompt generator concurrently
...
def stop_server():
    ...
    if stop_event and loop:
        # Schedule stop_event.set() on the event loop thread
